...
def smart_rule_exists(title: str) -> bool:
    ...
    expression = jmespath.compile(f"[?Title == '{title}'] | [0]")
    return expression.search(rules) is not None
...
def create_smart_rule(account_name: str, ip_address: str, row_index: int):
    ...
    if account_name.lower().startswith("pam"):
        ...
        expression = jmespath.compile(f"[?IPAddress == '{ip_address.strip()}'] | [0]")
        system = expression.search(systems)
        if not system:
    ...
    account = expression.search(accounts)
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            # İstersen burada get_existing_smart_rules.cache_clear() ekleyebilirsin
        else:
